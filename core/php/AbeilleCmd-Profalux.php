<?php
    ...
    // cmd: 00=Set Tilt, 01=Set Lift
    // $message['tilt']: Mandatory if cmd=00
    // $message['lift']: Mandatory if cmd=01
    // $message['duration']: Optional (default=10sec)
    function profaluxSetTiltLift($dest, $addr, $ep, $cmd, $message) {
        cmdLog2('debug', $addr, "  profaluxSetTiltLift(net=${net}, addr=${addr}, ep=${ep}, cmd=${cmd})");
        ...
        if ($cmd == "00") {
            ...
            if (!isset($message['tilt'])) {
                cmdLog2('error', $addr, "  Valeur 'tilt' manquante pour profaluxSetTiltLift()");
                return;
            }
        } else {
            $option = "01"; // Set lift only
            if (!isset($message['lift'])) {
                cmdLog2('error', $addr, "  Valeur 'lift' manquante pour profaluxSetTiltLift()");
                return;
            }
        }
        $lift = isset($message['lift']) ? isset($message['lift']) : 0;
        $tilt = isset($message['tilt']) ? isset($message['tilt']) : 0;
        $duration = isset($message['duration']) ? isset($message['duration']) : 10; // Default = 10sec
