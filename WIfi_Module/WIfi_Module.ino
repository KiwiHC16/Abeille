...
void loop(void)
{
  ...
  if (!client) {
    ...
    // Si pas de client je purge le buffer serie TTL to zigate
  ...
  if(client.connected()) {
    ...
    if(count > 0) {      
      ...
      // Les flush me pose probleme, voir à les suprimer
      Serial.flush();
    ...
    while(Serial.available() && bytes_read < BUFFER_SIZE) {
      ...
      // Debug
      // if ( serial_buf[bytes_read] < 10 ) Serial.print("0"); else Serial.print("");
      // Serial.print(serial_buf[bytes_read],HEX);
      // yield();
    ...
    // Debug
    // if ( bytes_read > 0 ) Serial.println();
