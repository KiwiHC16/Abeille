...
function message($message)
{
    ...
    $parameters['NE_Name'] = ($parameters['NE'] == '0000') ? 'Ruche' : array_search($parameters['NE'], $knownNE_local);

    $topicArray = explode("/", $message->topic);
    $parameters['Voisine'] = $topicArray[1];
    $parameters['Voisine_Name'] = array_search($topicArray[1], $knownNE_local);//$knownNE_local[$parameters['Voisine']];
...
foreach ($knownNE as $name => $neAddress) {
    deamonlog('debug', 'AbeilleLQI main: ' . $name . ' - ' . $neAddress);
    if ($neAddress['LQI_Done']== 'undefined' ||  $neAddress['LQI_Done'] == 0) {
        // echo "Let s do\n";
        $NE = $neAddress;
        $NE_continue = 1;
        collectInformation($client, $NE);
