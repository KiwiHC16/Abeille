...
Lorsqu'un objet est crée, une icône lui est associée. Lorsqu'un nouvel objet est ajouté dans le fichier JSON _plugins/Abeille/core/class/AbeilleObjetDefinition.json_, il est possible de lui attribuer une icône personnalisée.
...
Le premier terme correspond au nom de l'appareil tel qu'il est défini dans le fichier _AbeilleObjetDefinition.json_, le second sera le texte affiché.

Le fichier image au format png est a déposer dans le répertoire _plugins/Abeille/docs/images_
